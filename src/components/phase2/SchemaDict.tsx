...
export function SchemaDict({ schemas }: SchemaDictProps) {
  return (
    <div className="space-y-4">
      ...
      {schemas.map((schema) => (
        <div key={schema.artifactId} className="border border-border/60 rounded-2xl bg-white overflow-hidden">
          <div className="px-4 py-3 border-b border-border/40 bg-gradient-to-r from-indigo-50/50 to-transparent">
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                {schema.columns.map((col, i) => (
                  <tr key={col.columnName} className={`border-t border-border/30 ${i % 2 === 0 ? 'bg-white' : 'bg-muted/10'}`}>
                    <td className="px-4 py-2.5">
                      <div className="flex items-center gap-2">
                        {col.isPrimaryKey && (
                          <span title="Primary Key"><Key className="w-3.5 h-3.5 text-amber-500" /></span>
                        )}
                        {col.isForeignKey && (
                          <span title="Foreign Key"><Link2 className="w-3.5 h-3.5 text-cyan-500" /></span>
                        )}
                    ...
                    <td className="px-4 py-2.5 text-center">
                      {col.nullable ? (
                        <span className="text-amber-500 text-[10px] font-medium">NULL</span>
                      ) : (
                        <span className="text-emerald-500 text-[10px] font-medium">NOT NULL</span>
                      )}
                    </td>
                  </tr>
                ))}
          ...
          <div className="px-4 py-2.5 border-t border-border/40 bg-muted/20 flex items-center gap-4 text-[10px] text-muted-foreground">
            ...
            <span className="flex items-center gap-1">
              <Link2 className="w-3 h-3 text-cyan-500" /> Foreign Key
            </span>
            <span className="flex items-center gap-1">
              <AlertCircle className="w-3 h-3 text-indigo-500" /> {schema.columns.length} columns
            </span>
          </div>
        </div>
      ))}
...
function getTypeColor(type: string): string {
  if (type.includes("Varchar") || type.includes("string")) return "bg-emerald-50 text-emerald-700";
  if (type.includes("Decimal") || type.includes("Integer") || type.includes("int")) return "bg-blue-50 text-blue-700";
  if (type.includes("Enum")) return "bg-purple-50 text-purple-700";
  if (type.includes("FK") || type.includes("Foreign")) return "bg-cyan-50 text-cyan-700";
  if (type.includes("Timestamp") || type.includes("Date")) return "bg-amber-50 text-amber-700";
  if (type.includes("Computed") || type.includes("calculated")) return "bg-rose-50 text-rose-700";
