    ...
    <div className="space-y-5">
      ...
      {criticalConflicts.length > 0 && (
        <div className="bg-rose-50 border-2 border-rose-200 rounded-2xl p-4 flex items-start gap-3">
          ...
          <div>
            <h4 className="font-bold text-rose-800 text-sm">{criticalConflicts.length} Critical Conflicts Require Resolution</h4>
        ...
        {criticalConflicts.length > 0 && (
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <h4 className="font-semibold text-sm text-rose-800">Critical Conflicts</h4>
            </div>
        ...
        {warningConflicts.length > 0 && (
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <h4 className="font-semibold text-sm text-amber-800">Warnings</h4>
            </div>
            <div className="space-y-3">
      ...
      {/* Blocking Questions */}
      <div className="border-2 border-amber-200 rounded-2xl bg-amber-50/30 overflow-hidden">
        <div className="px-4 py-3 border-b border-amber-200/60 flex items-center gap-2 bg-amber-50">
          <HelpCircle className="w-4 h-4 text-amber-600" />
          <h4 className="font-semibold text-sm text-amber-900">Blocking Questions</h4>
        ...
        <div className="p-4 space-y-2">
          {blockingQuestions.map((question, i) => (
            <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-white border border-amber-100">
              <div className="w-6 h-6 rounded-lg bg-amber-100 text-amber-600 flex items-center justify-center text-xs font-bold flex-shrink-0">
                {i + 1}
              </div>
  ...
  return (
    <div className={`border-2 rounded-2xl overflow-hidden ${
      ...
      <div className="p-4 space-y-3">
        ...
        {/* Recommendation */}
        <div className="flex items-start gap-2 bg-white rounded-xl p-3 border border-indigo-100">
          <ArrowRight className="w-4 h-4 text-indigo-500 mt-0.5 flex-shrink-0" />
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider text-indigo-600 mb-0.5">Recommendation</p>
      ...
      <div className={`px-4 py-2 border-t flex items-center justify-between ${
        isCritical ? "border-rose-200 bg-rose-50" : "border-amber-200 bg-amber-50"
      }`}>
        <span className={`text-[10px] font-bold ${isCritical ? "text-rose-700" : "text-amber-700"}`}>
          {conflict.severity.toUpperCase()}
        </span>
        <span className={`text-[10px] font-medium ${conflict.resolved ? "text-emerald-600" : "text-muted-foreground"}`}>
