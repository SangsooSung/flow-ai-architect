    ...
    <div className="space-y-5">
      ...
      <div className="space-y-4">
        {flows.map((flow) => {
          ...
          return (
            <div key={flow.name} className="border border-border/60 rounded-2xl bg-card overflow-hidden">
              <button
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-900/50 text-emerald-700 dark:text-emerald-300">
                      ...
                      <div key={step.step} className="flex gap-3">
                        ...
                        <div className="flex-1 pb-3">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            {step.condition && (
                              <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-amber-50 dark:bg-amber-950/50 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800">
                          ...
                          {step.formula && (
                            <code className="block mt-2 text-[11px] bg-slate-900 dark:bg-slate-800 text-emerald-400 dark:text-emerald-300 px-3 py-2 rounded-lg font-mono leading-relaxed overflow-x-auto">
                              {step.formula}
                            </code>
                          )}
                        </div>
                  ...
                  <div className="border-t border-border/40 pt-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {flow.permissions.map((perm) => (
                        <div key={perm.role} className="p-3 rounded-xl bg-purple-50/30 dark:bg-purple-950/20 border border-purple-100 dark:border-purple-900">
                          <span className="text-xs font-bold text-purple-700 dark:text-purple-300 block mb-2">{perm.role}</span>
                          <div className="flex flex-wrap gap-1">
                            {perm.actions.map((action) => (
                              <span key={action} className="text-[10px] px-2 py-0.5 rounded-md bg-card border border-purple-200 dark:border-purple-800 text-purple-600 dark:text-purple-400">
                                {action}
                              </span>
                            ))}
                          </div>
